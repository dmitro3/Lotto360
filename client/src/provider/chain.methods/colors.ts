...
import { colorsContract } from "../contracts";
...
export const colorsChainMethods = {
    purchaseSpin: async (address: string, value: number, web3: Web3) => {
        try {
            return colorsContract(web3)
    ...
    userReadySpin: async (address: string, web3: Web3) => {
        try {
            return await colorsContract(web3).methods.GetReadySpin().call({ from: address });
        } catch (err) {
            console.error("Error checking ready spins:", err);
            return null;
        }
    ...
    userHistory: async (address: string, web3: Web3) => {
        try {
            return colorsContract(web3).methods.GetMyHistory().call({ from: address });
        } catch (err) {
            console.error("Error checking ready spins:", err);
            return null;
        }
    ...
    getSettingForUser: async (web3: Web3): Promise<UserSetting | null> => {
        try {
            const settings = await colorsContract(web3).methods.GetSettingForUser().call();
    ...
    getSpinById: async (id: string, address: string, web3: Web3) => {
        try {
            return colorsContract(web3).methods.UserGetSpinById(id).call({ from: address });
        } catch (err) {
            console.error("Error checking ready spins:", err);
            return null;
        }
...
export const colorsAdminChainMethods = {
    ...
    setContractFee: async (address: string, value: number, web3: Web3) => {
        try {
            return colorsContract(web3).methods.SetContractFee(value).send({
                from: address,
            });
        } catch (err) {
    ...
    setPrizeMultiplier: async (address: string, value: number, web3: Web3) => {
        try {
            return colorsContract(web3).methods.SetPrizeMultiplier(value).send({
                from: address,
            });
        } catch (err) {
    ...
    transferOwnership: async (address: string, newOwner: string, web3: Web3) => {
        try {
            return colorsContract(web3).methods.transferOwnership(newOwner).send({
                from: address,
            });
        } catch (err) {
    ...
    getSettingForAdmin: async (address: string, web3: Web3) => {
        try {
            return colorsContract(web3).methods.GetSettingForAdmin().call({
                from: address,
            });
        } catch (err) {
            console.error("Error transferring owner:", err);
    ...
    getSpins: async (address: string, web3: Web3) => {
        try {
            const result: any[] = await colorsContract(web3).methods.GetSpins().call({
                from: address,
            });
    ...
    getUserSpins: async (address: string, userAddress: string, web3: Web3) => {
        try {
            const result: any[] = await colorsContract(web3).methods.GetUserSpins(userAddress).call({
                from: address,
            });
    ...
    getSpinById: async (address: string, spinId: string, web3: Web3) => {
        try {
            const result: any[] = await colorsContract(web3).methods.GetSpinById(spinId).call({
                from: address,
            });
    ...
    withdrawToken: async (amount: string, address: string, to: string, web3: Web3) => {
        try {
            return await colorsContract(web3).methods.WithdrawToken(to, Web3.utils.toWei(amount, "ether")).send({
                from: address,
            });
        } catch (err) {
