...
const ColorsPurchase: FunctionComponent<ColorsPurchaseProps> = ({
    alreadyPurchased,
    balance,
    betAmount,
    btnSmallInfo,
    ...
    return (
        <div className={`purchase-box rounded bg-white shadow p-4 d-flex flex-column justify-content-between`}>
            ...
            <p>
                {btnSmallInfo}
                Win amount x{multiplier}:<span className="fw-bold ms-2">{betAmount * multiplier || 0} BNB</span>
            </p>
            <button
                type="button"
                className="btn btn-lg btn-success w-100 mt-3"
                disabled={!(betAmount >= minBet && betAmount <= maxBet) || alreadyPurchased || buttonLoading}
