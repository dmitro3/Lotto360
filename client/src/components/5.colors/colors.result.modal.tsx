...
const ColorsResultModal: FunctionComponent<ColorsResultModalProps> = ({ spin, showModal, setShowModal }) => {
    const { id, guess, result, purchaseTime, spinTime, amount, user } = spin;
    return (
        <Modal show={showModal || true} aria-labelledby="contained-modal-title-vcenter" centered>
            <Modal.Header>
                <Modal.Title id="contained-modal-title-vcenter">Spin information</Modal.Title>
            </Modal.Header>
            <Modal.Body>
                ...
                <div className="d-flex justify-content-between">
                    ...
                    <span className="fw-bold">
                        {moment(parseInt(purchaseTime) * 1000).format("DD/MM/YYYY - h:mm a")}
                    </span>
                </div>
                <div className="d-flex justify-content-between">
                    <span>Spin time: </span>
                    <span className="fw-bold">{moment(parseInt(spinTime) * 1000).fromNow()}</span>
                </div>
...
export default ColorsResultModal;

const calculatePrize = (spin: ColorsSpin) => {
